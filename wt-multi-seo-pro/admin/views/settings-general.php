...
<form method="post">
    ...
    <div class="plseo-card">
        ...
        <?php else : ?>
            <?php $archive_types_arr = array_values( (array) $archive_types ); ?>
            <nav class="plseo-cpt-tab-nav">
                <?php foreach ( $archive_types_arr as $idx => $post_type ) : ?>
                    <button type="button"
                        class="plseo-cpt-tab-btn<?php echo 0 === $idx ? ' is-active' : ''; ?>"
                        data-tab="<?php echo esc_attr( $post_type->name ); ?>">
            ...
            <?php foreach ( $archive_types_arr as $idx => $post_type ) : ?>
                ...
                <div class="plseo-cpt-tab-panel<?php echo 0 === $idx ? ' is-active' : ''; ?>"
                     data-tab="<?php echo esc_attr( $post_type->name ); ?>">
                    <div class="plseo-cpt-archive-block" data-default-lang="<?php echo esc_attr( $default_lang_slug ); ?>">
                        <?php $lang_keys = array_keys( $languages ); ?>
                        <div class="plseo-lang-tabs">
                            <nav class="plseo-lang-tab-nav">
                                <?php foreach ( $languages as $lang_slug => $lang_data ) : ?>
                                    <button type="button"
                                        class="plseo-lang-tab-btn<?php echo $lang_slug === $lang_keys[0] ? ' is-active' : ''; ?>"
                                        data-lang-tab="<?php echo esc_attr( $lang_slug ); ?>">
                                        <?php echo esc_html( $lang_data['name'] ?? strtoupper( (string) $lang_slug ) ); ?>
                                        <code><?php echo esc_html( $lang_slug ); ?></code>
                                    </button>
                                <?php endforeach; ?>
                            </nav>
                            <div class="plseo-lang-tab-panels">
                                <?php foreach ( $languages as $lang_slug => $lang_data ) : ?>
                                    <?php
                                    $lang_values = [];
                                    ...
                                    ?>
                                    <div class="plseo-lang-tab-panel<?php echo $lang_slug === $lang_keys[0] ? ' is-active' : ''; ?>"
                                         data-lang-tab="<?php echo esc_attr( $lang_slug ); ?>">
                                        <?php if ( $lang_slug !== $default_lang_slug ) : ?>
                                            <p class="plseo-lang-panel-actions">
                                                <button type="button" class="button button-secondary button-small plseo-copy-default-lang" data-target-lang="<?php echo esc_attr( $lang_slug ); ?>">
    ...
    <div class="plseo-card">
        <h2><?php esc_html_e( 'Site Verification', 'polylang-seo' ); ?></h2>
        <p class="description"><?php esc_html_e( 'Enter verification codes from search engines. Only the content value is needed, not the full meta tag.', 'polylang-seo' ); ?></p>
        <table class="form-table">
            <tr>
                <th><label for="plseo_google_site_verification"><?php esc_html_e( 'Google Search Console', 'polylang-seo' ); ?></label></th>
                <td><input type="text" class="large-text" id="plseo_google_site_verification" name="plseo_google_site_verification" value="<?php echo esc_attr( (string) PLSEO_Helpers::get_option( 'google_site_verification', '' ) ); ?>" placeholder="<?php esc_attr_e( 'Verification code only', 'polylang-seo' ); ?>" /></td>
            </tr>
            <tr>
                <th><label for="plseo_bing_site_verification"><?php esc_html_e( 'Bing Webmaster Tools', 'polylang-seo' ); ?></label></th>
                <td><input type="text" class="large-text" id="plseo_bing_site_verification" name="plseo_bing_site_verification" value="<?php echo esc_attr( (string) PLSEO_Helpers::get_option( 'bing_site_verification', '' ) ); ?>" /></td>
            </tr>
            <tr>
                <th><label for="plseo_yandex_site_verification"><?php esc_html_e( 'Yandex Webmaster', 'polylang-seo' ); ?></label></th>
                <td><input type="text" class="large-text" id="plseo_yandex_site_verification" name="plseo_yandex_site_verification" value="<?php echo esc_attr( (string) PLSEO_Helpers::get_option( 'yandex_site_verification', '' ) ); ?>" /></td>
            </tr>
            <tr>
                <th><label for="plseo_pinterest_site_verification"><?php esc_html_e( 'Pinterest', 'polylang-seo' ); ?></label></th>
                <td><input type="text" class="large-text" id="plseo_pinterest_site_verification" name="plseo_pinterest_site_verification" value="<?php echo esc_attr( (string) PLSEO_Helpers::get_option( 'pinterest_site_verification', '' ) ); ?>" /></td>
            </tr>
        </table>
    </div>
