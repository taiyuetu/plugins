jQuery(function ($) {
    ...
    $(document).on('click', '.plseo-cpt-tab-btn', function () {
        var $btn = $(this);
        var tab = $btn.data('tab');
        var $card = $btn.closest('.plseo-card');

        $card.find('.plseo-cpt-tab-nav .plseo-cpt-tab-btn').removeClass('is-active');
        $btn.addClass('is-active');
        $card.find('.plseo-cpt-tab-panel').removeClass('is-active');
        $card.find('.plseo-cpt-tab-panel[data-tab="' + tab + '"]').addClass('is-active');
    });

    $(document).on('click', '.plseo-lang-tab-btn', function () {
        var $btn = $(this);
        var lang = $btn.data('lang-tab');
        var $block = $btn.closest('.plseo-cpt-archive-block');

        $block.find('.plseo-lang-tab-btn').removeClass('is-active');
        $btn.addClass('is-active');
        $block.find('.plseo-lang-tab-panel').removeClass('is-active');
        $block.find('.plseo-lang-tab-panel[data-lang-tab="' + lang + '"]').addClass('is-active');
    });
